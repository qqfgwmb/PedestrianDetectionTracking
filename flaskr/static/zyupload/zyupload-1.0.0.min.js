(function($) {
	$.Jcrop = function(obj, opt) {
		...
		var $img2 = $("<img />").attr("src", $img.attr("src")).css("position", "absolute").width(boundx).height(
			boundy);
		...
		function interfaceUpdate(alt) {
			options.allowResize ? alt ? Selection.enableOnly() : Selection.enableHandles() : Selection
				.disableHandles();
			Tracker.setCursor(options.allowSelect ? "crosshair" : "default");
	...
	$.fn.zyPopup = function(options, param) {
		...
		return this.each(function() {
			...
			this.createHtml = function() {
				...
				zoom = $("#zoom").hide(), zoomContent = $("#zoom .content"), zoomedIn = false,
					openedImage = null, windowWidth = $(window).width(), windowHeight = $(window)
					.height()
			};
			this.openPopup = function() {
				...
				function render() {
					...
					if (imageWidth == zoomContent.width() && imageWidth <= maxImageWidth &&
						imageHeight == zoomContent.height() && imageHeight <= maxImageHeight) {
						show(image);
						return
					}
					zoomContent.animate({
	...
	$.fn.zyUpload = function(options, param) {
		...
		return this.each(function() {
			...
			this.createHtml = function() {
				...
				var html = "";
				if (para.dragDrop) {
					html += '<form id="uploadForm" action="' + para.url +
					...
					html += '					<a style="height:' + para.itemHeight + ";width:" + para
						.itemWidth +
						';" title="点击添加文件" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
					html += '						<div class="uploadImg" style="width:' + imgWidth + 'px">';
					html +=
						'							<img class="upload_image" src="zyupload/skins/images/add_img.png" style="width:expression(this.width > ' +
						imgWidth + " ? " + imgWidth + 'px : this.width)" />';
					html += "						</div>";
					html += "					</a>";
					html += "				</div>";
					html += "			</div>";
			...
			this.funDisposePreviewHtml = function(file, e) {
				...
				var delHtml = "";
				if (para.tailor) {
					editHtml = '<span class="file_edit" data-index="' + file.index +
						'" title="编辑"></span>'
				}
				if (para.del) {
					delHtml = '<span class="file_del" data-index="' + file.index +
						'" title="删除"></span>'
				}
				var newStr = file.name.split("").reverse().join("");
				var type = newStr.split(".")[0].split("").reverse().join("");
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '			<p class="file_name" title="' + file.name + '">' + file.name +
						"</p>";
					html += editHtml;
					html += delHtml;
					html += "		</div>";
					...
					html += '	<a style="height:' + para.itemHeight + ";width:" + para.itemWidth +
						';" href="#" class="imgBox">';
					html += '		<div class="uploadImg" style="width:' + imgWidth + "px;max-width:" +
						imgWidth + "px;max-height:" + imgHeight + 'px;">';
					...
					html += '	<p id="uploadTailor_' + file.index +
					...
					html += '		<div class="uploadImg" style="width:' + imgWidth + 'px">';
					html += '			<img id="uploadImage_' + file.index +
						'" class="upload_file" src="' + fileImgSrc +
						'" style="width:expression(this.width > ' + imgWidth + " ? " + imgWidth +
						'px : this.width)" />';
					html += "		</div>";
					html += "	</a>";
			...
			this.createPopupPlug = function(imgSrc, index, name) {
				$("body").zyPopup({
					...
					onTailor: function(val, quondamImgInfo) {
						...
						console.info(val);
						var tailor = "{'leftX':" + val.leftX + ",'leftY':" + val.leftY +
							",'rightX':" + val.rightX + ",'rightY':" + val.rightY +
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funBindDelEvent = function() {
							if ($(".file_del").length > 0) {
								$(".file_del").click(function() {
									ZYFILE.funDeleteFile(parseInt($(this).attr(
										"data-index")), true);
									return false
								})
							}
							if ($(".file_edit").length > 0) {
								$(".file_edit").click(function() {
									...
									var imgName = $(this).prev(".file_name")
										.attr("title");
									var imgSrc = $("#uploadImage_" + imgIndex)
										.attr("src");
									self.createPopupPlug(imgSrc, imgIndex,
										imgName);
									return false
								})
							}
						...
						var funBindHoverEvent = function() {
							$(".upload_append_list").hover(function(e) {
								...
								$(this).find(".file_bar").removeClass(
									"file_hover")
							})
						};
						funDealtPreviewHtml()
